...
`app/` is organized by domain, not by technical type — there is no shared `models.py`, `schemas.py`, or `routers/` bucketing every domain's files by kind. Each domain gets its own top-level folder owning everything about itself: `app/user/`, `app/goal/`, `app/profile/`, and `app/swim_time/` each hold `model.py` (SQLAlchemy), `schema.py` (Pydantic), and `router.py` (FastAPI endpoints). `app/stats/` is the same shape minus `model.py` — it's a read-only reporting domain with no entity of its own, only a `schema.py` + `router.py` querying across `user`/`swim_time`. This is a deliberate choice over a layered/hexagonal split (interface layer kept separate from domain logic): for a single-FastAPI-app CRUD service, a domain's model, schema, and router almost always change together for the same reason (the Common Closure Principle, Martin's *Agile Software Development*), so colocating them cuts navigation cost with no real flexibility given up.
