...
A comment must stand on its own. Never write a comment that points to another file or line to carry part of its meaning (e.g. "see app/auth/model.py:User.token_valid_after for why") — inline the actual reasoning at the comment site instead. If the same rationale is genuinely needed in multiple places, repeat the relevant sentence rather than cross-referencing; don't make one comment's correctness depend on another file staying put.
...
`app/` is organized by domain, not by technical type — there is no shared `models.py`, `schemas.py`, or `routers/` bucketing every domain's files by kind. Each domain gets its own top-level folder owning everything about itself: `app/auth/`, `app/goal/`, `app/profile/`, and `app/swim_time/` each hold `model.py` (SQLAlchemy), `schema.py` (Pydantic), and `router.py` (FastAPI endpoints). Domains are named after whichever the ubiquitous language actually calls the capability, not automatically after the entity a domain's model happens to define — `app/auth/` owns the `User` model, but is named for what its router actually does (register/login/logout/token issuance), not the entity it operates on; `app/goal/`, `app/profile/`, `app/swim_time/` are named after their entity because their routers genuinely manage that entity end-to-end. `app/stats/` is the same shape minus `model.py` — it's a read-only reporting domain with no entity of its own, only a `schema.py` + `router.py` querying across `auth`/`swim_time`. The domain-vs-layer split itself (grouping model+schema+router per domain) is a deliberate choice over a layered/hexagonal split (interface layer kept separate from domain logic): for a single-FastAPI-app CRUD service, a domain's model, schema, and router almost always change together for the same reason (the Common Closure Principle, Martin's *Agile Software Development*), so colocating them cuts navigation cost with no real flexibility given up.
